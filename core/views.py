...
def encurtar_link(request):
    ...
    if (form.is_valid()):
        try:
            form.save()
            return HttpResponse(f"Seu link foi criado com sucesso! Para acessar,acesse: https://127.0.0.1:8000/{link_encurtado}")
        except Exception as e:
            return HttpResponse("Erro interno do sistema: "+e)


def redirecionador_de_link(request, link_recebido):
    link = Links.objects.get(link_encurtado=link_recebido)
    if len(link) == 0:
        return redirect("/")
